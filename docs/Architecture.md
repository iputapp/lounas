...
注記: フローの1について、Supabase OTPメール送信からresend.comへの転送を行っています。
サービス稼働後期にはどのE-Mailでもログイン可能としています。
...
1. **フィルタリング条件**
   - `price`: 料理の価格上限 (650 / 850 / 1000 / 999999円) ※GUIの~650円,~850円,~1000円,おまかせ(すべての料理を含めるため)に対応。
   - `amount`: 量の特性スコア範囲 (small: 0-50 / medium: 50-75 / large: 75-100) ※GUIの少なめ,ふつう,がっつりに対応。おまかせは0-100でフィルタリング。
   - `commonality`: 個性の特性スコア範囲 (common: 0-45 / unique: 45-100) ※GUIの個性,定番に対応。おまかせは0-100でフィルタリング。
...
3. **レスポンス選択**
   ...
   - 5件未満: フィルタを緩めてランダム5件返却（要件に合致しないがUXのために妥協している）

## 月間店舗訪問数カウント

月間のユーザ1人の訪問数をホーム画面に表示する。プログレスバーおよび数値を表示。
訪問は、レコメンドのルート案内から「終了する」押下しダイアログ「はい」を選択したときのみ記録される。
この要件の意図は、訪問を記録することの制約が大きく、GUIの操作を前提とした。
その際、レコメンド機能と道案内機能を使用してほしいというアプリ作成時の要望とマッチしたため、この操作経路を暫定の記録とした。
