...
ビルからのルートが途中まで固定されており、途中から分岐するため連結リストを使用し統一する予定だったが、DB Seedingの制約により、固定ルートを直接挿入する形に変更。
その代わり、Seed時にサムネイル画像のIDを統一している。
...
ユーザーの所属組織 (大学・会社など)。
学生フラグ、教員フラグは当初学生のみに公開していた名残り。
将来的に建物ごとにサービス展開する予定であったため、ユーザに組織が割り当てられるようになっている。
※その場合、Restaurantsの移動時間のカラムは中間テーブルを用意するなどしてN-to-Mにする必要あり。
...
| `isStudent` | Boolean | - | 学生フラグ |
| `isStaff` | Boolean | - | 教員フラグ |
