...
export function TrianglesBottom({ move = false, zIndex = "-z-50" }: TrianglesBottomProps) {
  return (
    <div className={`${styles.bg} ${zIndex}`}>
      <span className={`${styles.triangle} ${styles.blue} ${move ? "move" : ""}`}></span>
      <span className={`${styles.triangle} ${styles.cyan} ${move ? "move" : ""}`}></span>
      <span className={`${styles.triangle} ${styles.green} ${move ? "move" : ""}`}></span>
    </div>
  );
}
